...
import type { LiveCallHistory } from "@/hooks/useProfile";

interface LiveCallHistoryPanelProps {
...
/**
 * Renders the user's live-call activity archive.
 *
 * Example:
 * ```tsx
 * <LiveCallHistoryPanel liveCalls={[]} />
 * ```
 */
export function LiveCallHistoryPanel({ liveCalls }: LiveCallHistoryPanelProps) {
  ...
  const completedCount = liveCalls.filter((liveCall) => liveCall.status === "won").length;
  const missedCount = liveCalls.filter((liveCall) => liveCall.status === "lost").length;
