                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium truncate">
                      {member.profile?.display_name || 'Utilisateur inconnu'}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Banni {formatDistanceToNow(new Date(member.banned_at), { addSuffix: true, locale: fr })}
                    </p>
                    {member.reason && (
                      <p className="text-xs text-destructive/80 mt-1 truncate">
                        Raison: {member.reason}
                      </p>
                    )}
                  </div>
                  <Button
                  ...
                  >
                    ...
                    {unbanning === member.user_id ? '...' : 'Débannir'}
                  </Button>
                </motion.div>
              ))
            )}
